...
void WriteFile(ofstream *f, int n) //Метод для генерации и записи структур в файл
{
    for (int i = 0; i < n; i++)
    {
        Country country;
        ...
        cin.ignore(); //Избавляемся от лишнего пробела, который появился после ввода курсы валюты
...
int main()
{
    ...
    ofstream OutputStream("F1.bin", ios::binary); //Открываем фаил в входном поток
    ...
    cin.ignore(); //Избавляемся от лишнего пробела, который появился после ввода кол-ва стран
